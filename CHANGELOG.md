...
- 2026-04-17: hydaelyn-mirror-v1 — added Hydaelyn system (rules/hydaelyn.md positioning + Hear/Feel/Think dual-loop intent) + first skill `/hyd-mirror` (skills/hyd-mirror/SKILL.md): intake four-step flow, mutual calibration with good/bad examples, `.hydaelyn/onboarded.json` first-time enforcement, `/hyd-mirror append` subcommand; sdd.md adds Trigger 1 spec-intake prompt; session-management.md adds Trigger 2 cross-day Mirror + skip-followup reminder; public repo renamed `claude-code-playbook` → `hydaelyn`
- 2026-04-17: knowledge-index — added "Tier Assignment Criteria" section: each tier gets one litmus test (Hot: will AI err in the next session's first three actions / Warm: affects specific tasks but not every session / Glacier: might someone ask three months from now) + positive/negative examples + Hot vs CLAUDE.md boundary paragraph; addresses external review P0.2 "three tiers collapse into one" concern
