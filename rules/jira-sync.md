...
| Purpose | Field | JIRA field |
|------|------|------------|
| Team | Project default | `customfield_10001` |
| Approver | — | `customfield_10003` |
| Start date | start date | `customfield_10015` |
| Target end | target end | `customfield_10023` |
...
- PHWEP FT-Task: `Todo` →(`製作中`)→ `In Progress` →(`製作完畢，提供組內確認`)→ `In Review` → `FINISH`
- AI large work: `Todo` → `In Progress` →(`RD製作完成`)→ `FINISH` (no In Review)
...
```
JIRA info check:
  Project key: ? (e.g., PHWEP, AI, UPD)
  Parent ticket: ? (e.g., PHWEP-38)
